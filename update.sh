...
# без токена GitHub API отдаёт 403 на раннерах Actions
auth=(); [ -n "${GITHUB_TOKEN:-}" ] && auth=(-H "Authorization: Bearer $GITHUB_TOKEN")
ver="${1:-$(curl -fsSL "${auth[@]}" https://api.github.com/repos/openai/codex/releases/latest | jq -r .tag_name | sed 's/^rust-v//')}"
[ -n "$ver" ] && [ "$ver" != "null" ] || { echo "could not determine latest version" >&2; exit 1; }

# system -> release target triple (package = bin/codex + codex-code-mode-host, codex-resources/bwrap, codex-path/rg)
...
for entry in $systems; do
  ...
  url="https://github.com/openai/codex/releases/download/rust-v$ver/codex-package-$target.tar.zst"
